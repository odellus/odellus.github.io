...
def get_posts():
    """Load and process posts - only called when directive runs"""
    import pandas as pd
...
def build_children(posts, number):
    """Build card children from posts"""
    if posts.empty:
        return []
        ...
        children.append(
            {
                ...
                "children": [
                    ...
                    {
                        "type": "paragraph",
                        "children": [{"type": "text", "value": irow["content"]}],
                        "data": {"needsMathParsing": True},
                    },
                    {
                        "type": "footer",
                        "children": [
...
def parse_math_expressions(text_value):
    ...
    for match in re.finditer(math_pattern, text_value):
        ...
        if match.start() > last_end:
            ...
            if before_text:
                nodes.append({"type": "text", "value": before_text})
    ...
    if last_end < len(text_value):
        ...
        if remaining_text:
            nodes.append({"type": "text", "value": remaining_text})
...
def run_directive(name, data):
    ...
    posts = get_posts()
    return build_children(posts, number)
